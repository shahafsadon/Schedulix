...
# Human-readable labels for each RankingCriterion, used by ranking combo-boxes.
...
class ScheduleNavigationScreen(ctk.CTkFrame):
    ...
    def __init__(
        ...
        # while generation is active (ranking priorities remain always enabled).
        ...
        # Ranking combo-box StringVar instances (one per priority slot).
    ...
    def _build_ranking_panel(self) -> None:
        """Build the Rankings & Filters panel below the main calendar/details area.

        Layout (row=3, full width)
        --------------------------
        Left column  — Ranking Priorities: three priority combo-boxes.
                       These are **always enabled**, even during generation.
        Right column — Threshold Constraints: min/max entry fields.
                       These are **disabled** while the runner is active and
                       re-enabled once generation goes idle.
        """
        panel = ctk.CTkFrame(
            self,
            fg_color=_SURFACE,
        ...
        # Restore the active ranking from the presenter so combos reflect
        # the ordering that was loaded from the cache at startup.
        active_criteria = list(self.presenter.current_ranking().criteria)
        ...
        for slot_index, slot_label in enumerate(priority_labels):
            ...
            # Pre-select the criterion stored in the presenter (if any).
            if slot_index < len(active_criteria):
                matching = [
                    k for k, v in _CRITERION_LABELS.items()
                    if v is active_criteria[slot_index]
                ]
                default_val = matching[0] if matching else _CRITERION_KEYS[0]
            else:
                default_val = _CRITERION_KEYS[0]  # "(none)"
            ...
            ).grid(row=3 + slot_index * 2, column=0, sticky="w", pady=(0, 8))
        ...
        # Sync the initial enable/disable state.
    ...
    def _handle_ranking_change(self) -> None:
        """Read priority combo-boxes, build RankingSettings, and re-rank.

        Cleaning rules applied to raw combo-box values:
        * "(none)" entries are mapped to None and skipped.
        * Duplicate criteria are removed (first occurrence wins) by
          ``RankingSettings.build()``.
        * The resulting settings are forwarded to the presenter, which sorts
          the in-memory buffer and fires ``on_ranking_changed`` so the cache
          is updated by the orchestration layer.
        """
        raw: list[RankingCriterion | None] = [
            _CRITERION_LABELS.get(var.get()) for var in self._ranking_vars
        ]
        settings = RankingSettings.build([c for c in raw if c is not None])
    ...
    def _refresh_threshold_state(self) -> None:
        ...
        Threshold constraints must not be changed while the background worker
        is active because the complete schedule set is not yet available.
        Ranking priorities always remain editable.
    ...
    def _refresh(self) -> None:
        ...
        # Always sync threshold controls with runner state.
        self._refresh_threshold_state()
