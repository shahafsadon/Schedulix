...
class RankingSettings:
    ...
    @classmethod
    def build(cls, criteria: list[RankingCriterion]) -> "RankingSettings":
        """Create a ``RankingSettings`` with duplicates removed.

        The first occurrence of each criterion is kept; subsequent
        duplicates are discarded.  ``None`` values in *criteria* are
        silently skipped so callers can pass raw, possibly-incomplete
        UI values.
